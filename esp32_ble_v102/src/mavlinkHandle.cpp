	...
	switch(state)
	{
		...
		case CONTROL_ANGLE_STATE_SET_MODE:
		{
			...
			}
			{
				if(ackCommand.command == MAV_CMD_USER_2)
				{
					gimbal_state_t gState;

					if(mode == LOCK_MODE) gState = GIMBAL_STATE_LOCK_MODE;
					else if(mode == FOLLOW_MODE) gState = GIMBAL_STATE_FOLLOW_MODE;
		...
		{
			static bool setGimbalAngle = false;
			static uint32_t timeSequence = 0;
			...
			{
				uint16_t deltaPan  = abs((int16_t)mavlink.mountOrientation.roll  - panSetpoint);
...
// 	{
// //		gimbal_param_test[i].value = getParamGimbalT3v3(i);
// //
// //		float param_value = gimbal_param_test[i].value;
// //		uint8_t len = sizeof(gimbal_param_test[i].param_id);
// //		char param_id[len];
// //
// //		memcpy(param_id, gimbal_param_test[i].param_id, len);
// 		float param_value = 2;
// 		char *param_id = "kinh";
